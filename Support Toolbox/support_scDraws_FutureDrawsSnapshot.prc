IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[support_scDraws_Snapshot]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[support_scDraws_Snapshot]
GO

CREATE PROCEDURE [dbo].[support_scDraws_FutureDrawsSnapshot]
AS
BEGIN
	set nocount on

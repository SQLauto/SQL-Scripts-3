IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[support_IndexMaintenance]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[support_IndexMaintenance]
GO
...
CREATE PROCEDURE [dbo].[support_IndexMaintenance]
	  @db_name nvarchar(50)
	, @fillfactor int = 100
...
*/
	...
	*/
	...
	set @msg = 'support_IndexMaintenance:  Procedure starting...'
	...
	*/
	...
	set @msg = 'support_IndexMaintenance:  Found ' + cast(@@rowcount as varchar) + ' objects to reorg/rebuild'
	exec syncSystemLog_Insert @ModuleId=2, @SeverityId=0, @CompanyId=1, @Message=@msg, @Source='IndexMaint'
	print @msg
	...
	while (1=1)
		begin;
			fetch next from partitions_cursor into @objectId, @ojbectName, @indexId, @indexName, @schemaName, @partitionNum, @frag;
			...
			begin
				set @command = N'ALTER INDEX ' + @indexName + N' ON ' + @schemaName + N'.' + @ojbectName + N' REORGANIZE';
			end	

			if ( @frag >= @fragThreshold_REBUILD_LowerLimit ) 
			begin
				set @command = N'ALTER INDEX ' + @indexName + N' ON ' + @schemaName + N'.' + @ojbectName + N' REBUILD';
			end	
	...
	set @msg = 'support_IndexMaintenance:  Procedure completed successfully.'
	exec syncSystemLog_Insert @ModuleId=2, @SeverityId=0, @CompanyId=1, @Message=@msg, @Source='IndexMaint'
	print @msg

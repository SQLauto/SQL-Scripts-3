...
, case 
	when last_run_date = 0 then ' - - '
	else SUBSTRING( cast(last_run_date as varchar), 5, 2)
		+ '/' + SUBSTRING( cast(last_run_date as varchar), 7, 2)
		+ '/' + SUBSTRING( cast(last_run_date as varchar), 1, 4)
	end 
		as [last_run_date]
, CASE s.freq_type
     WHEN  1 THEN 'Once'

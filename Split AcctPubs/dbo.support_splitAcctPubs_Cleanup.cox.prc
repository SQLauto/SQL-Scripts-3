IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[FixSplitManifestSequences]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[FixSplitManifestSequences]
GO
...
CREATE PROCEDURE [dbo].[FixSplitManifestSequences]
AS
BEGIN
	begin tran
...
from (  
	--|  splits
	select ManifestTemplateId, ManifestSequenceTemplateId, AccountId , COUNT(*) as [SplitCount]
	from (
		--|  prelim

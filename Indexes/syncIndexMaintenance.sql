...
FROM sys.dm_db_index_physical_stats(9, NULL, NULL, NULL , NULL) IPS 
...
BEGIN 
  ...
  BEGIN 
    ...
    RAISERROR(@Message, 0, 1) WITH NOWAIT 
    SET @cmd = 'ALTER INDEX all' + @IndexName + ' ON [' + RTRIM(LTRIM(@SchemaName)) + '].[' + RTRIM(LTRIM(@Table)) + '] REORGANIZE'  
		--EXEC (@cmd) 
		PRINT @cmd  
  ...
  BEGIN 
    ...
    RAISERROR(@Message, 0, 1) WITH NOWAIT 
    SET @cmd = 'ALTER INDEX All' + @IndexName + ' ON [' + RTRIM(LTRIM(@SchemaName)) + '].[' + RTRIM(LTRIM(@Table)) + '] REBUILD WITH (FILLFACTOR = ' + CONVERT(VARCHAR(3),@fillfactor) + ', STATISTICS_NORECOMPUTE = OFF)' 
		--EXEC (@cmd) 
		PRINT @cmd 

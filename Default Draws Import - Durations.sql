...
as (
	...
	from syncSystemLog
	where LogMessage like 'scDefaultDraws_Import found the following delivery dates within the source file%'
...
as (
	...
	from syncSystemLog
	where LogMessage like 'scDefaultDraws_Import completed successfully%'
	and SLTimeStamp > dateadd(d, -1*@daysBack, convert(varchar, getdate(),1))
)

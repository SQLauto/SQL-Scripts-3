...
/* PDA CONFIG */
	...
	if ( @doUpdate = 1 and @updateSignatureFilePath = 1 )
	begin
...
/* Exports */
		...
		select @sysPropertyValue = 
			case @ver 
				when '3.1.2.0002' then case @nestedCompanyFolders 
					when 1 then @installPath_Web + '\DataIO\' + @database + '\ExportConfig.xml'
					else @installPath_Web + '\DataIO\ExportConfig.xml'
					end
	...
	begin
		...
		select @sysPropertyValue =
			case @ver 
				when '3.1.2.0002' then case @nestedCompanyFolders 
					when 1 then @installPath_Web + '\DataIO\' + @database + '\DailyForecastExport.xml'
					else @installPath_Web + '\DataIO\DailyForecastExport.xml'
					end
				when '3.1.2.0004' then case @nestedCompanyFolders 
					when 1 then @installPath_Web + '\DataIO\' + @database + '\DailyForecastExport.xml'
					else @installPath_Web + '\DataIO\DailyForecastExport.xml'
	...
	begin
		...
		select @sysPropertyValue = 
			case @ver 
				when '3.1.2.0002' then case @nestedCompanyFolders 
					when 1 then @installPath_Web + '\DataIO\' + @database + '\InvoiceExportConfig.xml'
					else @installPath_Web + '\DataIO\InvoiceExportConfig.xml'
					end
				when '3.1.2.0004' then case @nestedCompanyFolders 
					when 1 then @installPath_Web + '\DataIO\' + @database + '\InvoiceExportConfig.xml'
					else @installPath_Web + '\DataIO\InvoiceExportConfig.xml'

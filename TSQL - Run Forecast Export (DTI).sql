...
set @sql = @sql + ' Custom "D:\Syncronex\SingleCopy\DataIO\CustomForecastExport_DTI.xml" /p "StartDate=' + convert(varchar, cast(@begindate as date),1) + '","StopDate=' + convert(varchar, cast(@enddate as date), 1) + '","UserID=' + cast(@userid as varchar) + '" '
